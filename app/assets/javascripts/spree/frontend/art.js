...
function Art() {
  ...
  var flkty, thumbnails_wrapper, thumbnails_wrapper_caution,flkty_tank,thumbnails_wrapper_tank,
      flkty_chair,flkty_road,flkty_caution,thumbnails_wrapper_chair,thumbnails_wrapper_road,thumbnails_wrapper_booth,flkty_booth;
  ...
  Art.prototype.index = function() {
	...
	thumbnails_wrapper_booth = document.querySelector('.thumbnails.booth');
    ...
    flkty_caution.on( 'cellSelect', function() {
      ...
      thumbnails_wrapper_caution.querySelectorAll('.thumbnail.caution').item(flkty_caution.selectedIndex).classList.add('current');
	  	   ...
	  	   flkty_booth = new Flickity('#artbook-booth', {
      cellAlign: 'center',
      cellSelector: '.slide',
      contain: true,
      imagesLoaded: true,
    ...
    flkty_booth.on( 'cellSelect', function() {
      thumbnails_wrapper_booth.querySelector('.current').classList.remove('current');
      thumbnails_wrapper_booth.querySelectorAll('.thumbnail.booth').item(flkty_booth.selectedIndex).classList.add('current');
    });
	...
	bind_thumbnail_events_road('.thumbnails');
	  bind_thumbnail_events_caution('.thumbnails');
	   bind_thumbnail_events_booth('.thumbnails');
  }
  // Private functions
  ...
  function bind_thumbnail_events(selector) {
    ...
    document.querySelector('.left.capture.booth').addEventListener('click', function(event){
      flkty_booth.previous();
    });

    document.querySelector('.right.capture.booth').addEventListener('click', function(event){
      flkty_booth.next();
    });

    document.querySelector('.thumbnails.booth').addEventListener('click', function(event){
      event.preventDefault();
      flkty_booth.select($(event.target.parentNode).index());
    }, true);
